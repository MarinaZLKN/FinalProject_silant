...
const ClaimFilter = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://127.0.0.1:8000/api/machines/').then((response) => {
      const serviceCompaniesSet = new Set(
        response.data.map((machine) => machine.service_company)
      );
      setServiceCompanies(Array.from(serviceCompaniesSet));
    });
  ...
  useEffect(() => {
    ...
    const filteredData = claims.filter((claim) => {
      const serviceCompanyMatch =
        !selectedServiceCompany || claim.machine.service_company === selectedServiceCompany;
